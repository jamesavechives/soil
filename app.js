...
//    });
  ...
  shareAppMessage: function (options) {
    var that = this;
    return {
      title: options.title || this.getAppTitle() || '蓝色理想开发',
      desc: options.desc || this.getAppDescription() || '蓝色理想开发，拖拽生成app，无需编辑代码，一键打包微信小程序',
      path: options.path,
      success: function () {
              ...
              var code = res['code'];
                wx.getUserInfo({
                  success: function (info) {
                    that._sendUserInfo(info,code,options);
                  },
                  fail: function (res) {
                    wx.showModal({
                      title: '提示',
  ...
  _sendUserInfo: function (info,code, options) {
    var that = this;
  ...
  pageDataInitial: function () {
                  ...
                  success: function (res) {
                    ...
                    if (param.form !== 'form') {
                      for (let j in res.data) {
                        ...
                        //  if (typeof description === 'string' && !/^http:\/\/img/g.test(description)) 
                          if (typeof description === 'string' && !/^http:\/\//g.test(description)) {
    ...
    if (!!pageInstance.carouselGroupidsParams) {
      ...
      //  let url = '/index.php?r=AppExtensionInfo/carouselPhotoProjiect';
        let url = '/AppInfo/carouselPhotos/?';
        pageInstance.requestNum = pageRequestNum + 1;
        _this.sendRequest({
          hideLoading: pageRequestNum++ == 1 ? false : true,
          url: url,
          ...
          _this.getLocation({
            success: function (res) {
              ...
              _this.sendRequest({
                ...
                url: '/products/getassesslist&idx_arr[idx]=goods_type&idx_arr[idx_value]=2',
    ...
    if (!!pageInstance.dynamicClassifyGroupidsParams) {
      ...
      for(let i = 0; i < params.length; i++){
        ...
        //  url: '/index.php?r=appData/getCategoryByGroup',
          url: '/products/getcategorybygroup',
          data: {
            group_id: groupId
          },
          success: function(res){
  ...
  _citylocationList: function (dataset, region_id) {
    ...
    let url = '/products/formdatalist&idx_arr[idx]=region_id&idx_arr[idx_value]='+region_id+'&extra_cond_arr[latitude]='+this.globalData.locationInfo.latitude+'&extra_cond_arr[longitude]='+this.globalData.locationInfo.longitude + '&extra_cond_arr[county_id]='+region_id,
        param = {'form':form};
    this.sendRequest({
      url: url,
      data: param,
  ...
  getValidateTostore: function () {
    ...
    url:'/orders/precheck_shoppingcart',
      data: {
        sub_shop_app_id: pageInstance.franchiseeId || '',
