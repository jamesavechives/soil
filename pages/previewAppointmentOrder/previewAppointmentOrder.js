...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({ is_group: options.is_group,to_store: options.to_store});
  },
  ...
  getCartList: function () {
    ...
    app.sendRequest({
      ...
      success: function(res){
        var data = [];
        var startDate,endDate,service_period,intervalDate;
        ...
        for (var i = 0; i <= data.length - 1; i++) {
          ...
          that.cart_data_arr.push({
            ...
            startDate = {
              month : data[0].appointment_start.substring(4,6),
              day : data[0].appointment_start.substring(6,8),
              hours : data[0].appointment_start.substring(8)
            }
            endDate =　{
              month : data[0].appointment_end.substring(4,6),
              day : data[0].appointment_end.substring(6,8),
              hours : data[0].appointment_end.substring(8)
            }
            intervalDate = endDate.day > startDate.day || endDate.month > startDate.month ? (endDate.month +'月'+ endDate.day + '日'):'当日'    
        service_period = startDate.month + '月' + startDate.day + '日 ' + startDate.hours +' - ' + intervalDate + endDate.hours;
        that.setData({
          goodsList: data,
    ...
    app.sendRequest({
      ...
      success: function(res){
        var info = res.data,
            ...
            need_user_address = goods_info[0].appointment_info.need_user_address,
        ...
        _this.setData({
          ...
          need_user_address: need_user_address,
        })
  ...
  confirmPayment: function(e){
    ...
    if (this.data.need_user_address == '0' &&!(this.data.appointment_user_phone && this.data.appointment_user_name)){
      app.showModal({
        content: '请填写个人信息'
      });
      return;
    ...
    app.sendRequest({
      ...
      data: {
        ...
        voucher_coupon_goods_info: this.data.exchangeCouponData.voucher_coupon_goods_info,
        appointment_to_store_time: this.data.to_store,
        appointment_user_phone: this.data.appointment_user_phone,
  ...
  payOrder: function(orderId){
    ...
    function paySuccess() {
      var pagePath = '/pages/goodsOrderPaySuccess/goodsOrderPaySuccess?detail=' + orderId + (that.franchisee_id ? '&franchisee='+that.franchisee_id : '') + '&is_group=' + !!that.is_group + '&code=code';
    ...
    function payFail(){
        app.turnToPage('/pages/appointmentOrderDetail/appointmentOrderDetail?detail=' + orderId, 1);
    }
  ...
  goToMyAddress: function () {
    ...
    app.turnToPage('/pages/myAddress/myAddress?id=' + addressId + '&need_address='+ this.data.need_user_address);
  },
  appointmentUserName: function(e){
    this.setData({
      appointment_user_name: e.detail.value
    })
  },
  appointmentUserPhone: function (e) {
    this.setData({
      appointment_user_phone: e.detail.value
    })
  },
