  ...
  getAppointmentList: function(param, initial){
    ...
    app.sendRequest({
      ...
      success: function(res){
        ...
        data['appointmentInfo.servicePrice'] = response.appointment_price + '元';
        that.setData(data);
      },
      fail:function(res){
        console.log(res)
    ...
    app.sendRequest({
      ...
      success: function(res){
        var cart_arr = [res.data],
            pagePath = '/pages/previewAppointmentOrder/previewAppointmentOrder?cart_arr='+ encodeURIComponent(cart_arr) + ('&to_store='+encodeURIComponent(time)) ;
